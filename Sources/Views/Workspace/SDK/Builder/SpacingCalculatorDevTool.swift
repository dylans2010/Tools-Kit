...
struct SpacingCalculatorDevTool: DevTool {
    ...
    func render() -> some View {
        SimpleDevToolView(title: name, placeholder: "Base unit (default: 4)") { input in
            let base = Double(input) ?? 4
            return (1...12).map { "Step \($0): \(Int(base * Double($0)))pt" }.joined(separator: "\n")
        }
    }
}
