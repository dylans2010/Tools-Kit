...
struct EpochConverterDevTool: DevTool {
    ...
    func render() -> some View {
        SimpleDevToolView(title: name, placeholder: "Enter Unix timestamp") { input in
            guard let interval = TimeInterval(input) else {
                return "Current: \(Int(Date().timeIntervalSince1970))\nDate: \(Date().formatted())"
            }
