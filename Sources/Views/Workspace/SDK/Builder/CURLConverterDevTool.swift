...
struct CURLConverterDevTool: DevTool {
    ...
    func render() -> some View {
        SimpleDevToolView(title: name, placeholder: "Paste cURL command") { input in
            var result = "// Swift URLRequest\n"
            let url = input.components(separatedBy: " ").first(where: { $0.hasPrefix("http") }) ?? "https://api.example.com"
            result += "var request = URLRequest(url: URL(string: \"\(url)\")!)\n"
