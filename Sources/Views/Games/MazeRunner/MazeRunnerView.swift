...
struct MazeRunnerView: View {
    @StateObject private var logic = MazeRunnerLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Maze Runner").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "square.grid.3x3.middle.filled").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Maze Runner").font(.title.bold()).foregroundColor(.white)
