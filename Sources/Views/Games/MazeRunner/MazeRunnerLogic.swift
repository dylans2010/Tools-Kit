...
final class MazeRunnerLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "maze_runner"
    ...
    @Published var showingSequence = true; @Published var sequenceLength = 5
    ...
    private let easySizes = [Int]([5])
    private let medSizes = [Int]([7])
    private let hardSizes = [Int]([9])

    func startGame(difficulty: Int = 0) {
        self.difficulty = difficulty
        sequenceLength = difficulty == 0 ? 5 : (difficulty == 1 ? 7 : 9)
