...
struct DominoesView: View {
    @StateObject private var logic = DominoesLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Dominoes").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "rectangle.split.2x1.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Dominoes").font(.title.bold()).foregroundColor(.white)
