...
struct TankAssaultView: View {
    @StateObject private var logic = TankAssaultLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Tank Assault").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "truck.box.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Tank Assault").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                if logic.consecutiveKills > 0 { Text("🔥\(logic.consecutiveKills)").font(.caption.bold()).foregroundColor(GamingDesignTokens.accentGold) }
                Spacer()
                Text("HP: \(logic.playerHP)").font(.caption.bold()).foregroundColor(logic.playerHP < 30 ? GamingDesignTokens.dangerRed : .white)
                Spacer()
                Text("🔫 \(logic.ammo)").font(.caption.bold()).foregroundColor(.white)
            }.padding(.horizontal)

            Text("Score: \(logic.score)").font(GamingDesignTokens.fontMono).foregroundColor(GamingDesignTokens.accentGold)

            ForEach(logic.enemiesAlive) { enemy in
                HStack {
                    Text("Lane \(enemy.lane + 1)").font(.caption2).foregroundColor(.white.opacity(0.5)).frame(width: 50)
                    ProgressView(value: Double(enemy.hp), total: Double(max(1, enemy.hp + 1))).tint(GamingDesignTokens.dangerRed)
                    Button("Fire") { logic.shoot(at: enemy) }
                        .font(.caption.bold()).foregroundColor(.white).padding(.horizontal, 12).padding(.vertical, 4)
                        .background(GamingDesignTokens.dangerRed, in: Capsule())
                }.padding(.horizontal)
            }

            HStack(spacing: 16) {
                Button("Reload (+5)") { logic.reload() }.font(.caption.bold()).foregroundColor(.black).padding(.horizontal, 16).padding(.vertical, 8).background(GamingDesignTokens.accentNeon, in: Capsule())
                Button("End Turn") { logic.enemyAttack() }.font(.caption.bold()).foregroundColor(.white).padding(.horizontal, 16).padding(.vertical, 8).background(Color.white.opacity(0.15), in: Capsule())
            }
            Spacer()
        }
    }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Total Kills", "\(logic.kills)")
                    statRow("Best Kill Streak", "\(logic.bestKillStreak)")
                    statRow("Shots Fired", "\(logic.shotsFired)")
                    statRow("Accuracy", logic.shotsFired > 0 ? String(format: "%.0f%%", Double(logic.kills) / Double(logic.shotsFired) * 100) : "N/A")
