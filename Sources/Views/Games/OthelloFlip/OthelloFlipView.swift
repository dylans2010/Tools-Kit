...
struct OthelloFlipView: View {
    @StateObject private var logic = OthelloFlipLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Othello Flip").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "circle.lefthalf.filled").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Othello Flip").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 8) {
            HStack {
                HStack(spacing: 4) { Circle().fill(.black).frame(width: 12); Text("\(logic.playerPieces)").font(.caption.bold()).foregroundColor(.white) }
                Spacer()
                Text(logic.currentPlayer == .black ? "Your Turn" : "AI...").font(.caption).foregroundColor(.white.opacity(0.6))
                Spacer()
                HStack(spacing: 4) { Circle().fill(.white).frame(width: 12); Text("\(logic.aiPieces)").font(.caption.bold()).foregroundColor(.white) }
                Spacer()
                ...
                ForEach(0..<8, id: \.self) { r in
                    HStack(spacing: 1) {
                        ForEach(0..<8, id: \.self) { c in
                            ZStack {
                                Rectangle().fill((r + c) % 2 == 0 ? Color.green.opacity(0.3) : Color.green.opacity(0.2))
                                if logic.board.count > r && logic.board[r].count > c {
                                    let p = logic.board[r][c]
                                    if p == .black { Circle().fill(.black).padding(3) }
                                    else if p == .white { Circle().fill(.white).padding(3) }
                                }
                            }.frame(width: 40, height: 40).onTapGesture { logic.placePiece(row: r, col: c) }
                        }
                    }
                }
            }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Your Pieces", "\(logic.playerPieces)")
                    statRow("AI Pieces", "\(logic.aiPieces)")
                    statRow("Moves", "\(logic.moveCount)")
                    statRow("Best Flip", "\(logic.bestFlip)")
