...
struct ManCalaMasterView: View {
    @StateObject private var logic = ManCalaMasterLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Mancala Master").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "oval.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Mancala Master").font(.title.bold()).foregroundColor(.white)
