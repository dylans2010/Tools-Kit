...
final class FruitSliceLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "fruit_slice"
    ...
    @Published var targets: [TargetItem] = []; @Published var sliced = 0
    ...
    func startGame(difficulty: Int = 0) {
        ...
        score = 0; sliced = 0; misses = 0; consecutiveHits = 0; bestStreak = 0
    ...
    func tap(_ item: TargetItem) {
        ...
        sliced += 1; consecutiveHits += 1; bestStreak = max(bestStreak, consecutiveHits)
    ...
    private func endGame() {
        gameTimer?.invalidate(); spawnTimer?.invalidate()
        won = sliced >= 10 + difficulty * 5; phase = .results
    }
    ...
    func finalReward() -> GameReward {
        ...
        if sliced >= 25 { badge = "Fruit Slice Star" }
