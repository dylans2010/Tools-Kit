...
struct FruitSliceView: View {
    @StateObject private var logic = FruitSliceLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Fruit Slice").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "leaf.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.successGreen)
                Text("Fruit Slice").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("Fruits Sliced: \(logic.sliced)").font(.caption.bold()).foregroundColor(GamingDesignTokens.successGreen)
                Spacer()
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Fruits Sliced", "\(logic.sliced)")
