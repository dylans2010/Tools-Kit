...
struct NavalCombatView: View {
    @StateObject private var logic = NavalCombatLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Naval Combat").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "ferry.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Naval Combat").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 8) {
            HStack {
                Text("Shots: \(logic.shotsRemaining)").font(.caption.bold()).foregroundColor(.white)
                Spacer()
                if logic.consecutiveHits > 0 { Text("🔥\(logic.consecutiveHits)").font(.caption.bold()).foregroundColor(GamingDesignTokens.accentGold) }
                Spacer()
                Text("Hits: \(logic.hits)").font(.caption.bold()).foregroundColor(GamingDesignTokens.successGreen)
                Spacer()
                Text("Score: \(logic.score)").font(GamingDesignTokens.fontMono).foregroundColor(GamingDesignTokens.accentGold)
            }.padding(.horizontal)

            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 1), count: logic.gridSize), spacing: 1) {
                ForEach(0..<logic.gridSize * logic.gridSize, id: \.self) { idx in
                    let r = idx / logic.gridSize; let c = idx % logic.gridSize
                    let cell = logic.grid.count > r && logic.grid[r].count > c ? logic.grid[r][c] : .unknown
                    Rectangle()
                        .fill(cellColor(cell))
                        .frame(height: max(24, CGFloat(280 / logic.gridSize)))
                        .overlay(cellIcon(cell))
                        .onTapGesture { logic.fireAt(row: r, col: c) }
                }
    ...
    private func cellColor(_ c: NavalCombatLogic.CellState) -> Color {
        switch c {
        case .unknown: return GamingDesignTokens.cardSurface
        case .miss: return Color.blue.opacity(0.3)
        case .hit: return GamingDesignTokens.dangerRed.opacity(0.6)
        case .sunk: return GamingDesignTokens.dangerRed
        }
    }

    private func cellIcon(_ c: NavalCombatLogic.CellState) -> some View {
        Group {
            switch c {
            case .miss: Text("•").font(.caption2).foregroundColor(.white.opacity(0.4))
            case .hit: Text("X").font(.caption2.bold()).foregroundColor(.white)
            default: EmptyView()
            }
        }
    }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Hits", "\(logic.hits)")
                    statRow("Misses", "\(logic.misses)")
                    statRow("Accuracy", logic.hits + logic.misses > 0 ? String(format: "%.0f%%", Double(logic.hits) / Double(logic.hits + logic.misses) * 100) : "N/A")
