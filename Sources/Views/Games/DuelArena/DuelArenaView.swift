...
struct DuelArenaView: View {
    @StateObject private var logic = DuelArenaLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Duel Arena").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "person.2.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Duel Arena").font(.title.bold()).foregroundColor(.white)
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Duels Won", "\(logic.duelsWon)")
                    statRow("Duels Lost", "\(logic.duelsLost)")
                    statRow("Best Reaction", String(format: "%.0fms", logic.bestReaction))
                    statRow("Perfect Draws", "\(logic.perfectDraws)")
