...
struct SymbolSnapView: View {
    @StateObject private var logic = SymbolSnapLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Symbol Snap").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "star.square.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Symbol Snap").font(.title.bold()).foregroundColor(.white)
