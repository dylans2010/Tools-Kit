...
final class SnakeClassicLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "snake_classic"
    ...
    @Published var targets: [TargetItem] = []; @Published var applesEaten = 0
    ...
    func startGame(difficulty: Int = 0) {
        ...
        score = 0; applesEaten = 0; misses = 0; consecutiveHits = 0; bestStreak = 0
    ...
    func tap(_ item: TargetItem) {
        ...
        applesEaten += 1; consecutiveHits += 1; bestStreak = max(bestStreak, consecutiveHits)
    ...
    private func endGame() {
        gameTimer?.invalidate(); spawnTimer?.invalidate()
        won = applesEaten >= 10 + difficulty * 5; phase = .results
    }
    ...
    func finalReward() -> GameReward {
        ...
        if applesEaten >= 25 { badge = "Snake Classic Star" }
