...
final class SpaceInvaderLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "space_invader"
    ...
    func startGame(difficulty: Int = 0) {
        ...
        lives = 3 + difficulty * 20; maxLives = lives
    ...
    func endTurn() {
        ...
        lives -= dmg; consecutiveKills = 0
        streakMultiplier = max(1.0, streakMultiplier - 0.1)
        if lives <= 0 { lives = 0; won = false; phase = .results }
    }
    ...
    func finalReward() -> GameReward {
        ...
        if totalKills >= 25 { badge = "Space Invader Champion" }
