...
struct SpaceInvaderView: View {
    @StateObject private var logic = SpaceInvaderLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Space Invader").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "sparkle").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Space Invader").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("HP: \(logic.lives)").font(.caption.bold()).foregroundColor(logic.lives < 30 ? GamingDesignTokens.dangerRed : .white)
                Spacer()
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Aliens Destroyed", "\(logic.aliensDestroyed)")
                    statRow("Lives Left", "\(logic.lives)")
                    statRow("Level", "\(logic.level)")
