...
struct CrapsKingView: View {
    @StateObject private var logic = CrapsKingLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Craps King").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "die.face.5.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentGold)
                Text("Craps King").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 16) {
            HStack {
                Text("Roll \(logic.round)/\(logic.totalRounds)").font(.caption.bold()).foregroundColor(GamingDesignTokens.accentNeon)
                Spacer()
            ...
            Text("🎲 \(logic.currentValue)").font(.system(size: 60)).foregroundColor(.white)
            if !logic.roundResult.isEmpty { Text(logic.roundResult).font(.headline).foregroundColor(GamingDesignTokens.accentGold) }
            if !logic.roundOver {
                HStack(spacing: 20) {
                    Button("7 or 11") { logic.resolve(logic.currentValue == 7 || logic.currentValue == 11, payout: logic.betAmount * 3) }.font(.headline).foregroundColor(.black).padding(.horizontal, 20).padding(.vertical, 10).background(GamingDesignTokens.accentNeon, in: Capsule())
                    Button("Roll") { logic.currentValue = Int.random(in: 2...12); logic.resolve(logic.currentValue == 7 || logic.currentValue == 11, payout: logic.betAmount * 2) }.font(.headline).foregroundColor(.black).padding(.horizontal, 20).padding(.vertical, 10).background(GamingDesignTokens.accentGold, in: Capsule())
                }
            } else {
                Button("Next Roll") { logic.nextRound() }.font(.headline).foregroundColor(.black).padding(.horizontal, 30).padding(.vertical, 10).background(GamingDesignTokens.accentNeon, in: Capsule())
            }
            Spacer()
        }
    }
