...
struct FortressDefendView: View {
    @StateObject private var logic = FortressDefendLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Fortress Defend").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "building.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Fortress Defend").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("HP: \(logic.wallHP)").font(.caption.bold()).foregroundColor(logic.wallHP < 30 ? GamingDesignTokens.dangerRed : .white)
                Spacer()
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Attackers Defeated", "\(logic.attackersDefeated)")
                    statRow("Wall HP", "\(logic.wallHP)/\(logic.maxWallHP)")
                    statRow("Waves", "\(logic.wave)")
                    statRow("Arrows Used", "\(logic.arrowsUsed)")
