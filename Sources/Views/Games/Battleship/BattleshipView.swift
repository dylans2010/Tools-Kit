...
struct BattleshipView: View {
    @StateObject private var logic = BattleshipLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Battleship").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "water.waves").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Battleship").font(.title.bold()).foregroundColor(.white)
