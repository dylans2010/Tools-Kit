...
final class RhythmTapLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "rhythm_tap"
    ...
    @Published var targets: [TargetItem] = []; @Published var perfectHits = 0
    ...
    func startGame(difficulty: Int = 0) {
        ...
        score = 0; perfectHits = 0; misses = 0; consecutiveHits = 0; bestStreak = 0
    ...
    func tap(_ item: TargetItem) {
        ...
        perfectHits += 1; consecutiveHits += 1; bestStreak = max(bestStreak, consecutiveHits)
    ...
    private func endGame() {
        gameTimer?.invalidate(); spawnTimer?.invalidate()
        won = perfectHits >= 10 + difficulty * 5; phase = .results
    }
    ...
    func finalReward() -> GameReward {
        ...
        if perfectHits >= 25 { badge = "Rhythm Tap Star" }
