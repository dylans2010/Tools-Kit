...
struct RhythmTapView: View {
    @StateObject private var logic = RhythmTapLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Rhythm Tap").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "music.note").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Rhythm Tap").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("Perfect Hits: \(logic.perfectHits)").font(.caption.bold()).foregroundColor(GamingDesignTokens.successGreen)
                Spacer()
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 6), count: 3), spacing: 6) {
                ForEach(0..<12, id: \.self) { pos in
                    ...
                    Button { if let t = active { logic.tap(t) } } label: {
                        Circle()
                            .fill(active != nil ? GamingDesignTokens.accentPurple : GamingDesignTokens.cardSurface)
                            .frame(height: 70)
                    }
                }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Perfect Hits", "\(logic.perfectHits)")
