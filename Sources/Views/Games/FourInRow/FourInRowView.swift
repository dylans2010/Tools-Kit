...
struct FourInRowView: View {
    @StateObject private var logic = FourInRowLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Four in a Row").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "circle.grid.2x1.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Four in a Row").font(.title.bold()).foregroundColor(.white)
