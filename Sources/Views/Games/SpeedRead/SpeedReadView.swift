...
struct SpeedReadView: View {
    @StateObject private var logic = SpeedReadLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Speed Read").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "book.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Speed Read").font(.title.bold()).foregroundColor(.white)
