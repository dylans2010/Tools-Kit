...
struct BallBounceView: View {
    @StateObject private var logic = BallBounceLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Ball Bounce").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "circle.circle.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentGold)
                Text("Ball Bounce").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("Hoops Scored: \(logic.hoopsScored)").font(.caption.bold()).foregroundColor(GamingDesignTokens.successGreen)
                Spacer()
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 6), count: 3), spacing: 6) {
                ForEach(0..<12, id: \.self) { pos in
                    ...
                    Button { if let t = active { logic.tap(t) } } label: {
                        Circle()
                            .fill(active != nil ? GamingDesignTokens.accentGold : GamingDesignTokens.cardSurface)
                            .frame(height: 70)
                    }
                }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Hoops Scored", "\(logic.hoopsScored)")
