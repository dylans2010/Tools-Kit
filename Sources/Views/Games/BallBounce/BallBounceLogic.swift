...
final class BallBounceLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "ball_bounce"
    ...
    @Published var targets: [TargetItem] = []; @Published var hoopsScored = 0
    ...
    func startGame(difficulty: Int = 0) {
        ...
        score = 0; hoopsScored = 0; misses = 0; consecutiveHits = 0; bestStreak = 0
    ...
    func tap(_ item: TargetItem) {
        ...
        hoopsScored += 1; consecutiveHits += 1; bestStreak = max(bestStreak, consecutiveHits)
    ...
    private func endGame() {
        gameTimer?.invalidate(); spawnTimer?.invalidate()
        won = hoopsScored >= 10 + difficulty * 5; phase = .results
    }
    ...
    func finalReward() -> GameReward {
        ...
        if hoopsScored >= 25 { badge = "Ball Bounce Star" }
