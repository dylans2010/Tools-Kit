...
struct VideoPokerProView: View {
    @StateObject private var logic = VideoPokerProLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Video Poker Pro").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "rectangle.stack.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentGold)
                Text("Video Poker Pro").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 16) {
            ...
            if !logic.roundOver {
                Button("Deal") { logic.flipCoin() }.font(.headline).foregroundColor(.black).padding(.horizontal, 30).padding(.vertical, 10).background(GamingDesignTokens.accentGold, in: Capsule())
            } else {
                Button("Next Hand") { logic.nextRound() }.font(.headline).foregroundColor(.black).padding(.horizontal, 30).padding(.vertical, 10).background(GamingDesignTokens.accentNeon, in: Capsule())
            }
            Spacer()
        }
    }
