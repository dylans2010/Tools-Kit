...
struct TargetShootView: View {
    @StateObject private var logic = TargetShootLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Target Shoot").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "target").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Target Shoot").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("Targets Hit: \(logic.targetsHit)").font(.caption.bold()).foregroundColor(GamingDesignTokens.successGreen)
                Spacer()
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 6), count: 3), spacing: 6) {
                ForEach(0..<12, id: \.self) { pos in
                    ...
                    Button { if let t = active { logic.tap(t) } } label: {
                        Circle()
                            .fill(active != nil ? GamingDesignTokens.dangerRed : GamingDesignTokens.cardSurface)
                            .frame(height: 70)
                    }
                }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Targets Hit", "\(logic.targetsHit)")
