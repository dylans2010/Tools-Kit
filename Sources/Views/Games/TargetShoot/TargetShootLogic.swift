...
final class TargetShootLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "target_shoot"
    ...
    @Published var targets: [TargetItem] = []; @Published var targetsHit = 0
    ...
    func startGame(difficulty: Int = 0) {
        ...
        score = 0; targetsHit = 0; misses = 0; consecutiveHits = 0; bestStreak = 0
    ...
    func tap(_ item: TargetItem) {
        ...
        targetsHit += 1; consecutiveHits += 1; bestStreak = max(bestStreak, consecutiveHits)
    ...
    private func endGame() {
        gameTimer?.invalidate(); spawnTimer?.invalidate()
        won = targetsHit >= 10 + difficulty * 5; phase = .results
    }
    ...
    func finalReward() -> GameReward {
        ...
        if targetsHit >= 25 { badge = "Target Shoot Star" }
