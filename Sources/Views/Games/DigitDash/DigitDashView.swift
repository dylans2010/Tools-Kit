...
struct DigitDashView: View {
    @StateObject private var logic = DigitDashLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Digit Dash").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "textformat.123").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Digit Dash").font(.title.bold()).foregroundColor(.white)
