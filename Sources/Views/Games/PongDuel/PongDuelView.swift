...
struct PongDuelView: View {
    @StateObject private var logic = PongDuelLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Pong Duel").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "minus.rectangle.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Pong Duel").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("Rallies Won: \(logic.ralliesWon)").font(.caption.bold()).foregroundColor(GamingDesignTokens.successGreen)
                Spacer()
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 6), count: 3), spacing: 6) {
                ForEach(0..<12, id: \.self) { pos in
                    ...
                    Button { if let t = active { logic.tap(t) } } label: {
                        Circle()
                            .fill(active != nil ? GamingDesignTokens.accentNeon : GamingDesignTokens.cardSurface)
                            .frame(height: 70)
                    }
                }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Rallies Won", "\(logic.ralliesWon)")
