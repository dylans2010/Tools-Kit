...
final class PongDuelLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "pong_duel"
    ...
    @Published var targets: [TargetItem] = []; @Published var ralliesWon = 0
    ...
    func startGame(difficulty: Int = 0) {
        ...
        score = 0; ralliesWon = 0; misses = 0; consecutiveHits = 0; bestStreak = 0
    ...
    func tap(_ item: TargetItem) {
        ...
        ralliesWon += 1; consecutiveHits += 1; bestStreak = max(bestStreak, consecutiveHits)
    ...
    private func endGame() {
        gameTimer?.invalidate(); spawnTimer?.invalidate()
        won = ralliesWon >= 10 + difficulty * 5; phase = .results
    }
    ...
    func finalReward() -> GameReward {
        ...
        if ralliesWon >= 25 { badge = "Pong Duel Star" }
