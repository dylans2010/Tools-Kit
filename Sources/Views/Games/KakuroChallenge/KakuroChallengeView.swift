...
struct KakuroChallengeView: View {
    @StateObject private var logic = KakuroChallengeLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Kakuro Challenge").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "plus.rectangle.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Kakuro Challenge").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 12) {
            HStack {
                Text("Level \(logic.level)/\(logic.totalLevels)").font(.caption.bold()).foregroundColor(GamingDesignTokens.accentNeon)
                Spacer()
                Text("Moves: \(logic.moves)").font(.caption2).foregroundColor(.white.opacity(0.6))
                Spacer()
                if logic.hints > 0 {
                    Button("Hint (\(logic.hints))") { logic.useHint() }.font(.caption2.bold()).foregroundColor(.white).padding(4).background(GamingDesignTokens.accentPurple, in: Capsule())
                }
                Spacer()
                Text("Score: \(logic.score)").font(GamingDesignTokens.fontMono).foregroundColor(GamingDesignTokens.accentGold)
            }.padding(.horizontal)

            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 4), count: logic.gridSize), spacing: 4) {
                ForEach(0..<logic.gridSize * logic.gridSize, id: \.self) { idx in
                    let r = idx / logic.gridSize; let c = idx % logic.gridSize
                    let val = logic.grid.count > r && logic.grid[r].count > c ? logic.grid[r][c] : 0
                    let correct = logic.solution.count > r && logic.solution[r].count > c ? logic.solution[r][c] : -1
                    Button { logic.selectCell(row: r, col: c) } label: {
                        Text("\(val)").font(.headline).foregroundColor(val == correct ? GamingDesignTokens.successGreen : .white)
                            .frame(maxWidth: .infinity).frame(height: max(36, CGFloat(240 / logic.gridSize)))
                            .background(val == correct ? GamingDesignTokens.successGreen.opacity(0.15) : GamingDesignTokens.cardSurface, in: RoundedRectangle(cornerRadius: 6))
                    }
                }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Levels Solved", "\(logic.level)")
                    statRow("Total Moves", "\(logic.moves)")
