...
struct MissileLaunchView: View {
    @StateObject private var logic = MissileLaunchLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Missile Launch").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "location.north.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Missile Launch").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("HP: \(logic.citiesRemaining)").font(.caption.bold()).foregroundColor(logic.citiesRemaining < 30 ? GamingDesignTokens.dangerRed : .white)
                Spacer()
            ...
            ForEach(logic.enemies) { enemy in
                HStack {
                    Text(enemy.name).font(.caption).foregroundColor(.white).frame(width: 50, alignment: .leading)
                    ProgressView(value: Double(enemy.hp), total: Double(max(1, enemy.hp + 1))).tint(GamingDesignTokens.dangerRed)
                    Button("Attack") { logic.attack(enemy) }
                        .font(.caption.bold()).foregroundColor(.white).padding(.horizontal, 10).padding(.vertical, 4)
                        .background(GamingDesignTokens.dangerRed, in: Capsule())
                }.padding(.horizontal)
            }

            Button("End Turn") { logic.endTurn() }.font(.caption.bold()).foregroundColor(.white).padding(.horizontal, 20).padding(.vertical, 8).background(Color.white.opacity(0.15), in: Capsule())
            Spacer()
        }
    }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Missiles Intercepted", "\(logic.intercepted)")
                    statRow("Cities Saved", "\(logic.citiesRemaining)/\(logic.totalCities)")
                    statRow("Waves", "\(logic.wave)")
                    statRow("Accuracy", String(format: "%.0f%%", logic.accuracy))
