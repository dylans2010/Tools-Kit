...
struct BackgammonView: View {
    @StateObject private var logic = BackgammonLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Backgammon").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "triangle.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentNeon)
                Text("Backgammon").font(.title.bold()).foregroundColor(.white)
