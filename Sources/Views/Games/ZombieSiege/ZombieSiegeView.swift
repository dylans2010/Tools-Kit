...
struct ZombieSiegeView: View {
    @StateObject private var logic = ZombieSiegeLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Zombie Siege").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "figure.walk").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Zombie Siege").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("HP: \(logic.barricadeHP)").font(.caption.bold()).foregroundColor(logic.barricadeHP < 30 ? GamingDesignTokens.dangerRed : .white)
                Spacer()
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Zombies Killed", "\(logic.totalKills)")
                    statRow("Barricade HP", "\(logic.barricadeHP)/\(logic.maxBarricadeHP)")
                    statRow("Waves", "\(logic.wave)")
                    statRow("Headshots", "\(logic.headshots)")
