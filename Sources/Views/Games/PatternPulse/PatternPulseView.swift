...
struct PatternPulseView: View {
    @StateObject private var logic = PatternPulseLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Pattern Pulse").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "waveform.circle.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Pattern Pulse").font(.title.bold()).foregroundColor(.white)
