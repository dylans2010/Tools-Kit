...
final class PatternPulseLogic: ObservableObject, GamesRewardable {
    let gameIdentifier = "pattern_pulse"
    ...
    @Published var showingSequence = true; @Published var sequenceLength = 4
    ...
    private let easySizes = [Int]([4])
    private let medSizes = [Int]([6])
    private let hardSizes = [Int]([8])

    func startGame(difficulty: Int = 0) {
        self.difficulty = difficulty
        sequenceLength = difficulty == 0 ? 4 : (difficulty == 1 ? 6 : 8)
