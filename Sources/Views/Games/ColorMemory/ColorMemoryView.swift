...
struct ColorMemoryView: View {
    @StateObject private var logic = ColorMemoryLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Color Memory").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "circle.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Color Memory").font(.title.bold()).foregroundColor(.white)
