...
struct WhackAMoleView: View {
    @StateObject private var logic = WhackAMoleLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Whack-a-Mole").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "hand.point.up.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.dangerRed)
                Text("Whack-a-Mole").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                if logic.consecutiveWhacks > 0 { Text("🔥\(logic.consecutiveWhacks)").font(.caption.bold()).foregroundColor(GamingDesignTokens.accentGold) }
                Spacer()
                Text("Whacked: \(logic.whacked)").font(.caption.bold()).foregroundColor(GamingDesignTokens.successGreen)
                Spacer()
                Text("Score: \(logic.score)").font(GamingDesignTokens.fontMono).foregroundColor(GamingDesignTokens.accentGold)
            }.padding(.horizontal)

            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 6), count: 3), spacing: 6) {
                ForEach(0..<logic.gridSize, id: \.self) { pos in
                    Button { logic.whack(pos) } label: {
                        Circle()
                            .fill(logic.molePositions.contains(pos) ? GamingDesignTokens.dangerRed : GamingDesignTokens.cardSurface)
                            .frame(height: 70)
                            .overlay(logic.molePositions.contains(pos) ? Text("🐹").font(.title) : nil)
                    }
                }
        ...
        return ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ...
                    statRow("Moles Whacked", "\(logic.whacked)")
                    statRow("Missed", "\(logic.missed)")
