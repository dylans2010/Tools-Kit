...
struct CoinFlipDuelView: View {
    @StateObject private var logic = CoinFlipDuelLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Coin Flip Duel").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "bitcoinsign.circle.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentGold)
                Text("Coin Flip Duel").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 16) {
            HStack {
                Text("Flip \(logic.round)/\(logic.totalRounds)").font(.caption.bold()).foregroundColor(GamingDesignTokens.accentNeon)
                Spacer()
            ...
            Image(systemName: "bitcoinsign.circle.fill").font(.system(size: 80)).foregroundColor(GamingDesignTokens.accentGold)
            if !logic.roundResult.isEmpty { Text(logic.roundResult).font(.headline).foregroundColor(GamingDesignTokens.accentGold) }
            if !logic.roundOver {
                Button("Flip!") { logic.flipCoin() }.font(.title2.bold()).foregroundColor(.black).padding(.horizontal, 40).padding(.vertical, 14).background(GamingDesignTokens.accentGold, in: Capsule())
            } else {
                Button("Next Flip") { logic.nextRound() }.font(.headline).foregroundColor(.black).padding(.horizontal, 30).padding(.vertical, 10).background(GamingDesignTokens.accentNeon, in: Capsule())
            }
            Spacer()
        }
    }
