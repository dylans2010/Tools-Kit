...
struct SoundMatchView: View {
    @StateObject private var logic = SoundMatchLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Sound Match").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "speaker.wave.2.fill").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentPurple)
                Text("Sound Match").font(.title.bold()).foregroundColor(.white)
