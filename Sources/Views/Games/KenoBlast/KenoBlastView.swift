...
struct KenoBlastView: View {
    @StateObject private var logic = KenoBlastLogic()
    ...
    var body: some View {
        ...
        .navigationTitle("Keno Blast").navigationBarTitleDisplayMode(.inline)
    ...
    private var lobbyView: some View {
        ScrollView {
            VStack(spacing: 20) {
                Image(systemName: "circle.grid.3x3").font(.system(size: 64)).foregroundColor(GamingDesignTokens.accentGold)
                Text("Keno Blast").font(.title.bold()).foregroundColor(.white)
    ...
    private var gameView: some View {
        VStack(spacing: 12) {
            HStack {
                Text("Draw \(logic.round)/\(logic.totalRounds)").font(.caption.bold()).foregroundColor(GamingDesignTokens.accentNeon)
                Spacer()
                Text("Balance: \(logic.balance)").font(GamingDesignTokens.fontMono).foregroundColor(GamingDesignTokens.accentGold)
            }.padding(.horizontal)
            Text("Pick \(5 + logic.difficulty * 2) numbers").font(.caption).foregroundColor(.white.opacity(0.6))
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 4), count: 8), spacing: 4) {
                ForEach(1...40, id: \.self) { n in
                    Button("\(n)") { logic.selectNumber(n) }
                        .font(.caption.bold()).foregroundColor(logic.drawnNumbers.contains(n) ? .black : .white)
                        .frame(maxWidth: .infinity).frame(height: 32)
                        .background(logic.drawnNumbers.contains(n) && logic.selectedNumbers.contains(n) ? GamingDesignTokens.successGreen : (logic.selectedNumbers.contains(n) ? GamingDesignTokens.accentNeon : GamingDesignTokens.cardSurface), in: RoundedRectangle(cornerRadius: 4))
                }
            }.padding(.horizontal, 8)
            if !logic.roundResult.isEmpty { Text(logic.roundResult).font(.headline).foregroundColor(GamingDesignTokens.accentGold) }
            if !logic.roundOver {
                Button("Draw!") { logic.draw() }.font(.headline).foregroundColor(.black).padding(.horizontal, 30).padding(.vertical, 10).background(GamingDesignTokens.accentGold, in: Capsule())
            } else {
