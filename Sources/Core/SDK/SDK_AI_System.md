...
# SDK AI Reliability & Anti-Hallucination Rules (Mandatory)

These rules are mandatory for any AI system that answers questions about ToolsKit SDK:

1. **Source-of-truth boundary**: Only use facts explicitly documented in this file and SDK source files under `Sources/Core/SDK/`.
2. **No fabricated APIs**: Never invent modules, symbols, methods, imports, packages, protocols, or behaviors that are not present in the SDK source.
3. **SDK import correctness**: This project is an SDK, not a Swift Package dependency named `ToolsKit`. Responses must not instruct users to `import ToolsKit` unless such a module name is explicitly defined by the SDK target configuration.
4. **Evidence-first responses**: Every technical claim must be traceable to concrete SDK implementation details (types, method names, files, behavior).
5. **Uncertainty protocol**: If a requested behavior is not represented in the SDK sources, respond with “not implemented / not defined in current SDK sources” instead of guessing.
6. **Runtime behavior constraints**: Describe lifecycle, permissions, security, routing, connector, plugin, and module behavior exactly as implemented; do not generalize from external frameworks.
7. **Change awareness**: If repository state changes, regenerate this document from current `Sources/Core/SDK` before answering architecture questions.
8. **Conflict rule**: If this document and code disagree, code in `Sources/Core/SDK` wins; update this document immediately.

---

## Merged Root Inventory Reference
