...
public final class SDKProjectManager: ObservableObject {
    ...
    public func createProject(name: String, description: String = "", status: SDKProject.ProjectStatus = .draft) -> SDKProject {
        let ownerId = AuthorizationManager.shared.authSession?.userId ?? "unregistered"
        let project = SDKProject(name: name, description: description, status: status, ownerIdentifier: ownerId)
