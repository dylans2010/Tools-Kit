...
public final class SDKPermissionManager {
    ...
    public func validateProjectScopes(_ project: SDKProjectLegacy) throws {
        for scope in project.requiredScopes {
            if !isScopeAuthorized(scope) {
                SDKLogStore.shared.log("Permission denied for scope: \(scope)", source: "SDKPermissionManager", level: .error)
    ...
    public func grantPermission(_ scope: String) {
        ...
        persistPermissions()
        SDKLogStore.shared.log("Permission granted: \(scope)", source: "SDKPermissionManager", level: .info)
    }

    public func revokePermission(_ scope: String) {
        grantedPermissions.remove(scope)
        persistPermissions()
        SDKLogStore.shared.log("Permission revoked: \(scope)", source: "SDKPermissionManager", level: .info)
    }
