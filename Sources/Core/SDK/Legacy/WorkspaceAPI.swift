...
public final class WorkspaceAPI {
    ...
    struct NotesAPI {
        ...
        func createNote(title: String, content: String) -> Note {
            ...
            SDKLogStore.shared.log("Note created via WorkspaceAPI: \(title)", source: "WorkspaceAPI.Notes", level: LogLevel.info)
    ...
    struct TasksAPI {
        ...
        func createTask(title: String, dueDate: Date?) -> WorkspaceTask {
            ...
            SDKLogStore.shared.log("Task created via WorkspaceAPI: \(title)", source: "WorkspaceAPI.Tasks", level: LogLevel.info)
            return task
        }
    }
    let tasks = TasksAPI()
    ...
    struct MailAPI {
        ...
        func sendMail(to: String, subject: String, body: String) async throws {
            ...
            SDKLogStore.shared.log("Mail sent via WorkspaceAPI to \(to)", source: "WorkspaceAPI.Mail", level: LogLevel.info)
        }
    }
    let mail = MailAPI()
    ...
    struct CalendarAPI {
        ...
        func createEvent(title: String, start: Date, end: Date) {
            ...
            SDKLogStore.shared.log("Calendar event created via WorkspaceAPI: \(title)", source: "WorkspaceAPI.Calendar", level: LogLevel.info)
        }
    }
    ...
    struct FilesAPI {
        ...
        func deleteFile(id: String) {
            ...
            if FileManager.default.fileExists(atPath: path) {
                ...
                SDKLogStore.shared.log("File deleted via WorkspaceAPI: \(path)", source: "WorkspaceAPI.Files", level: LogLevel.info)
            } else {
                SDKLogStore.shared.log("File not found for deletion: \(path)", source: "WorkspaceAPI.Files", level: LogLevel.warning)
            }
        }
    }
    let files = FilesAPI()
    ...
    struct SlidesAPI {
        ...
        func createDeck(title: String) {
            ...
            SDKLogStore.shared.log("Slide deck created via WorkspaceAPI: \(title)", source: "WorkspaceAPI.Slides", level: LogLevel.info)
        }
        ...
        func generateContent(deckID: UUID, prompt: String) async throws {
            ...
            SDKLogStore.shared.log("Slide content generated for \(deck.title): \(response.prefix(50))...", source: "WorkspaceAPI.Slides", level: LogLevel.info)
        }
    }
    ...
    struct MeetAPI {
        func startMeeting(title: String) async throws -> String {
            ...
            if let roomURL = await DailyService.shared.internalRoomURL(for: session) {
                SDKLogStore.shared.log("Meeting started via WorkspaceAPI: \(title) at \(roomURL)", source: "WorkspaceAPI.Meet", level: LogLevel.info)
                return roomURL.absoluteString
            }
            SDKLogStore.shared.log("Meeting started via WorkspaceAPI: \(title) (ID: \(session.meetingId))", source: "WorkspaceAPI.Meet", level: LogLevel.info)
    ...
    struct TimeTravelAPI {
        ...
        func restoreState(snapshotID: UUID) throws {
            ...
            SDKLogStore.shared.log("Snapshot restored via WorkspaceAPI: \(snapshotID)", source: "WorkspaceAPI.TimeTravel", level: LogLevel.info)
        }

        func createSnapshot(message: String) {
            TimeTravelManager.shared.takeSnapshot(message: message)
            SDKLogStore.shared.log("Snapshot created via WorkspaceAPI: \(message)", source: "WorkspaceAPI.TimeTravel", level: LogLevel.info)
        }
    }
    ...
    struct PersonaAPI {
        func queryPersona(prompt: String) async throws -> String {
            ...
            SDKLogStore.shared.log("Persona query via WorkspaceAPI", source: "WorkspaceAPI.Persona", level: LogLevel.info)
            return response
        }
        ...
        func injectMemory(entityID: UUID, content: String) {
            ...
            SDKLogStore.shared.log("Persona memory injected for \(entityID)", source: "WorkspaceAPI.Persona", level: LogLevel.info)
        }
    }
    ...
    struct IntegrationsAPI {
        func executeWorkflow(workflowID: UUID) async throws {
            if let workflow = UnifiedDataStore.shared.integrationWorkflows.first(where: { $0.id == workflowID }) {
                ...
                SDKLogStore.shared.log("Workflow executed via WorkspaceAPI: \(workflowID)", source: "WorkspaceAPI.Integrations", level: LogLevel.info)
            } else {
        ...
        func triggerWorkflow(event: String) {
            ...
            SDKLogStore.shared.log("Workflow triggered for event: \(event)", source: "WorkspaceAPI.Integrations", level: LogLevel.info)
        }
    }
