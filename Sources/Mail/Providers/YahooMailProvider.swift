...
final class YahooMailProvider: NSObject, MailProvider, ASWebAuthenticationPresentationContextProviding {
    ...
    func authenticate(credentials: MailCredentials) async throws -> MailSession {
        let clientID = try oauthValue(key: "YAHOO_OAUTH_CLIENT_ID")
        let redirectURI = try oauthValue(key: "YAHOO_OAUTH_REDIRECT_URI")
    ...
    private func oauthValue(key: String) throws -> String {
        guard let value = Bundle.main.object(forInfoDictionaryKey: key) as? String,
            ...
            throw NSError(domain: "YahooProvider", code: 500, userInfo: [NSLocalizedDescriptionKey: "Missing \(key)"])
        }
        return value
    }
