...
class YahooMailProvider: MailProviderProtocol {
    ...
    func fetchThreads(in folder: MailFolder, limit: Int, offset: Int) async throws -> [MailThread] {
        ...
        try await imapService.connect(provider: .yahoo)
    ...
    func sendMessage(_ message: MailMessage) async throws {
        ...
        try await smtpService.send(message: message, user: account.email, pass: password, provider: .yahoo)
    }
