...
class OutlookMailProvider: MailProviderProtocol {
    ...
    func fetchThreads(in folder: MailFolder, limit: Int, offset: Int) async throws -> [MailThread] {
        ...
        try await imapService.connect(provider: .outlook)
    ...
    func sendMessage(_ message: MailMessage) async throws {
        ...
        try await smtpService.send(message: message, user: account.email, pass: password, provider: .outlook)
    }
